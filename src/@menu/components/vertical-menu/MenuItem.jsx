...
const MenuItem = (props, ref) => {
  ...
  const {
    ...
    // activeUrl,
    activeUrls = [], // Accept an array of URLs
    pathname, // Current pathname passed from the parent
    component,
    onActiveChange,
    rootStyles,
    ...rest
  ...
  // Helper function to check if pathname matches any activeUrls
  const matchesActiveUrl = url => {
    if (url.endsWith('*')) {
      // Handle wildcard: check if pathname starts with the base path (remove '*')
      const basePath = url.slice(0, -1)
  ...
  // Determine active state based on activeUrls
  const isActive = activeUrls.some(matchesActiveUrl)
