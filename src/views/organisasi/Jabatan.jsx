...
import { createJabatan, dataJabatan, deleteJabatan, findJabatan, updateJabatan } from '@/utils/jabatan'
...
export default function Jabatan() {
  ...
  useEffect(() => {
    dataJabatan(paginationModel.page + 1, paginationModel.pageSize, searchQuery)
      .then(response => {
        if (response.status) {
  ...
  const handleDelete = async (id, nama) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await dataJabatan(paginationModel.page + 1, paginationModel.pageSize, searchQuery)
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await updateJabatan(formData.id, formData)
  ...
  return (
    <>
      <Typography variant='h3'>Jabatan</Typography>
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 2 }}>
        {/* Card Form */}
        <Card sx={{ flex: { xs: '1', md: '30%' }, padding: 2, maxHeight: { md: 200 } }}>
          <CardContent>
            <Typography variant='h6'>Form Jabatan</Typography>
        ...
        <Card sx={{ flex: { xs: '1', md: '70%' } }}>
          <CardContent>
            <Typography variant='h6'>Data Jabatan</Typography>
