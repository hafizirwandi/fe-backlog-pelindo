...
export default function Divisi() {
  // Form
  const [formData, setFormData] = useState({ id: '', unit_id: '', nama: '' })
  ...
  useEffect(() => {
    dataDivisi(paginationModel.page + 1, paginationModel.pageSize, searchQuery)
      .then(response => {
        if (response.status) {
  ...
  const handleDelete = async (id, nama) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await dataDivisi(paginationModel.page + 1, paginationModel.pageSize, searchQuery)
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await updateDivisi(formData.id, formData)
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await createDivisi(formData)
  ...
  return (
    <>
      ...
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 2 }}>
        {/* Card Form */}
        <Card sx={{ flex: { xs: '1', md: '30%' }, padding: 2, maxHeight: { md: 300 } }}>
          <CardContent>
            <Typography variant='h6'>Form Divisi</Typography>
              ...
              {isEdit ? (
                <Stack direction='row' spacing={1}>
                  ...
                  <Button
                    ...
                    color='secondary'
                    onClick={() => {
                      setFormData({ id: '', nama: '', unit_id: '' }) // Reset form
        ...
        <Card sx={{ flex: { xs: '1', md: '70%' } }}>
          <CardContent>
            <Typography variant='h6'>Data Divisi</Typography>
